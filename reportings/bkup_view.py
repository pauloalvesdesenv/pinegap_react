...
def homepage_dashboard_view(request):
    ...
    global_stats = {
        ...
        "scans": {
           ...
           #"time_minutes": ScanDefinition.objects.filter(period='minutes', scan_type='periodic'),
           #"time_hours": ScanDefinition.objects.filter(period='hours', scan_type='periodic'),
           #"time_seconds": ScanDefinition.objects.filter(period='seconds', scan_type='periodic'),
           #"time_days": ScanDefinition.objects.filter(period='days', scan_type='periodic'),
           # "recorrency": ScanDefinition.objects.filter(period=[0], scan_type='periodic'),
        },
        "engines": {
...
def patch_management_view(request):
    ...
    dataset_7days = Asset.objects.filter(
        ...
        Q(rawfinding__risk_info__vuln_publication_date__gte=seven_days_ago.strftime('%d/%m/%Y')) &
    ...
    dataset_30days = Asset.objects.filter(
        ...
        Q(rawfinding__risk_info__vuln_publication_date__gte=month_ago.strftime('%d/%m/%Y')) &
        Q(rawfinding__risk_info__cvss_base_score__gte=7.0) &
        Q(rawfinding__type__in=ness_plugin_family_osupdates)
        #Q(rawfinding__scan__engine_policy_id=1)
    ).distinct()
...
def list_assets_view(request):
    ...
    allowed_sort_options = ["id", "name", "criticity_num", "financeiro" "score", "type",
                            ...
                            "-id", "-name", "-criticity_num", "-financeiro", "-score",
