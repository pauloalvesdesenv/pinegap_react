# -*- coding: utf-8 -*-
from __future__ import unicode_literals  #add corrigir problema nome do scan
...
#            new_name = '/'.join(['policies', self.engine.name, os.path.basename(initial_path)])
            new_path = os.path.join(MEDIA_ROOT, 'policies',
                self.engine.name, str(self.owner.id), os.path.basename(initial_path))
		#self.engine.name, os.path.basename(initial_path))
