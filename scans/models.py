...
class ScanDefinition(models.Model):
    ...
    title            = models.CharField(editable=True, max_length=256, blank=True)
    description      = models.CharField(max_length=512, blank=True, null=True)  #256 - 512
    ...
    engine_type      = models.ForeignKey(Engine, null=True)
    engine           = models.ForeignKey(EngineInstance, null=True, blank=True) #Force scan instance
    engine_policy    = models.ForeignKey(EnginePolicy, null=True)
...
class Scan(models.Model):
    ...
    title           = models.CharField(editable=True, max_length=256,  null=True, blank=True)
    status          = models.CharField(max_length=20)
    engine          = models.ForeignKey(EngineInstance, null=True, blank=True)
    engine_type     = models.ForeignKey(Engine,null=True)
    engine_policy   = models.ForeignKey(EnginePolicy, null=True)
    owner           = models.ForeignKey(User, null=False)
...
class ScanCampaign(models.Model):
    ...
    title           = models.CharField(max_length=512)   #from 256 - 512
    description     = models.CharField(max_length=512)   #from 256 - 512
    enabled         = models.BooleanField(default=False)
    status          = models.CharField(max_length=20)
    owner           = models.ForeignKey(User, null=False)
    timeout_delay   = models.IntegerField(null=True, blank=True)
    report_filepath = models.CharField(max_length=256, null=True, blank=True)  #space mas_leng and null=true
    scheduled_at    = models.DateTimeField(null=True, blank=True)
    expire_at       = models.DateTimeField(null=True, blank=True)
    created_at      = models.DateTimeField(default=timezone.now)
    updated_at      = models.DateTimeField(default=timezone.now)
